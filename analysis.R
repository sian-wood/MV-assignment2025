...
meanvar.plot(mvabund(train[,43:47]), 
             cex = 1, 
             pch = 19, 
             cex.axis = 0.9, 
             ylab = "Variance", 
...
#' (Warton 2005)
set.seed(12338)
mod4 <- manyglm(train_mvabund[,43:47] ~ pcscores_mvabund[,2:7], 
...
anova(mod4, p.uni = "adjusted", resamp = "montecarlo")
summary(mod4)
...
mod6 <- manyglm(train_mvabund[,43:47] ~ train_nocor_mvabund[,2:28], 
                family = "negative_binomial")
plot(mod6) # Okay...
anova(mod6) # Vars definitely explain some of the var in the data
anova(mod6, p.uni = "adjusted")
summary(mod6) # Nope
